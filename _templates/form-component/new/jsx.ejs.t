...
export const <%= h.inflection.camelize(name.replace(/\s/g, '_')) %> = () => (state, actions, {firstName, lastName} = state.<%= h.inflection.camelize(name.replace(/\s/g, '_'), true) %> || {}) => (
  <form class="<%= h.inflection.dasherize(name.toLowerCase()) %>" key="<%= h.inflection.dasherize(name.toLowerCase()) %>" method="post">
    ...
    <input type="text" name="firstName" oninput={ev => update<%= h.inflection.camelize(name.replace(/\s/g, '_')) %>({firstName: ev.target.value})(state, actions)} />
    <input type="text" name="lastName" oninput={ev => update<%= h.inflection.camelize(name.replace(/\s/g, '_')) %>({lastName: ev.target.value})(state, actions)} />
  </form>
)